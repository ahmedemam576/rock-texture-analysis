...
img=io.imread(r'C:\Users\49174\OneDrive\Desktop\innovation project\1st_pic.jpg', as_gray=True)
...
yellow_area = np.sum(binary == 1) ##yellow color

purple_area = np.sum(binary == 0) ##purple area

segments_ratio = yellow_area/purple_area

plt.pie([yellow_area,purple_area],labels=['yellow_segment','purple_segment'])

###################################################
# with the next setup we could detect the main orientioan of the texture which is inclined
#in the direction pi/4.2 = 43 degree
import numpy as np
import cv2
import matplotlib.pyplot as plt

ksize = 23  #Use size that makes sense to the image and fetaure size. Large may not be good. 
#On the synthetic image it is clear how ksize affects imgae (try 5 and 50)
sigma = 3 #Large sigma on small features will fully miss the features. 
theta = -1*np.pi/4.2  #/4 shows horizontal 3/4 shows other horizontal. Try other contributions
...
import cv2
import matplotlib.pyplot as plt
image = cv2.imread(r'C:\Users\49174\OneDrive\Desktop\1st_pic.jpg')
